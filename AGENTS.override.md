...
本文件只放当前项目的专属约束。团队通用行为仍以 `AGENTS.md` 和 `.codex/rules/` 为准。
...
- 必读文档：`README.md`、`00_总览/Windows应用安全基线画像库 AI 维护规范.md`、`AGENTS.md`、`00_总览/Windows应用安全基线画像库总览.md`、`00_总览/维护工作流.md`、对应类型模板（`99_模板/*`）、相关实体页面、`logs/变更日志.md`。

## 项目专属质量策略

- 需要优先遵守的产品不变量：
  - Markdown / Obsidian 是唯一正式知识源；`out/` 中的 SQLite、JSONL、Cypher 是派生产物，不得当作主数据源手工编辑。
...
- 允许破坏兼容的边界：可以调整 Markdown 模板与 `tools/` 抽取脚本；但改动 frontmatter 字段、`type` 枚举、目录结构或 API 输出契约时，必须同步 `README.md`、`00_总览/Windows应用安全基线画像库 AI 维护规范.md`、`11_抽取与图谱/` 抽取规范和样例数据。
...
- 必须按需使用的项目 skill：`.codex/skills/wiki-maintenance/SKILL.md`（新增 / 更新画像、补安全基线、变更日志、结构化抽取影响）。
- 通用工程行为：`.codex/skills/project-skill/SKILL.md`。
- 配置治理：涉及 AGENTS / rules / hooks / skill 噪声或重复时使用 `.codex/skills/codex-config-optimizer`。

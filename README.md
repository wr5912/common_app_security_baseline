...
> **平台维度**：每个画像页面用 frontmatter `os: windows | linux | cross` 标注平台。应用 / 服务 / 进程 / 父子关系 / 启动方式 / 文件 / 网络 / 安全基线等目录跨平台共享，靠 `os` 字段区分；平台专属持久化位置分目录承载——Windows 用 `06_注册表画像/`，Linux 用 `12_Linux持久化与配置/`。跨平台方法论页用 `os: cross`（例如 [[服务持久化机制对比]]）。
...
- `00_总览/终端应用安全基线画像库 AI 维护规范.md` 明确了 AI 的维护规则、更新流程、禁止事项；
- `99_模板/` 提供统一写作骨架；
- `11_抽取与图谱/` 提供结构化抽取路径。
...
| `config_persistence` | `12_Linux持久化与配置/` | Linux 持久化 / 配置位置画像 |
...
- `00_总览/终端应用安全基线画像库 AI 维护规范.md`：AI 维护规范；

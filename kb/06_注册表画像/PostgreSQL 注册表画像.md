...
本页记录 [[PostgreSQL]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。

## 2. 常见字段

```text
HKCU\SOFTWARE\PostgreSQL Global Development Group\*
HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*
HKLM\SOFTWARE\PostgreSQL Global Development Group\*
HKLM\SYSTEM\CurrentControlSet\Services\postgresql-x64 服务模式
HKLM\SYSTEM\CurrentControlSet\Services\postgresql-x64-*
```

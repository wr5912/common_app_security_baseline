...
os: windows
hive: HKLM/HKCU
key_pattern: "Cisco AnyConnect 注册表画像"
purpose:
  - service_config
  - app_config
  - uninstall_inventory
...
本页记录 [[Cisco AnyConnect]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。

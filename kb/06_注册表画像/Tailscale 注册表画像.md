...
本页记录 [[Tailscale]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。
...
HKLM\SYSTEM\CurrentControlSet\Services\tailscaled
```

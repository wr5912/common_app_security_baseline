...
本页记录 [[向日葵远控]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。
...
HKLM\SOFTWARE\Oray\*
HKLM\SYSTEM\CurrentControlSet\Services\AweSun 服务模式

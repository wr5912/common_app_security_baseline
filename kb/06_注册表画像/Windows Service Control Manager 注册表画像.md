...
os: windows
hive: HKLM/HKCU
key_pattern: "Windows Service Control Manager 注册表画像"
purpose:
  - service_config
  - app_config
  - uninstall_inventory
...
本页记录 [[Windows Service Control Manager]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。

## 2. 常见字段

```text
HKCU\SOFTWARE\Microsoft Corporation\*
HKLM\SOFTWARE\Microsoft Corporation\*
HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*
无独立服务时不适用；见证据页 not_applicable_types
```

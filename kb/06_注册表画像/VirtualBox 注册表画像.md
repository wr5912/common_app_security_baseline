...
本页记录 [[VirtualBox]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。
...
HKLM\SYSTEM\CurrentControlSet\Services\VBoxNetDHCP
HKLM\SYSTEM\CurrentControlSet\Services\VBoxNetLwf
HKLM\SYSTEM\CurrentControlSet\Services\VBoxSDS
HKLM\SYSTEM\CurrentControlSet\Services\VirtualBox DHCP Service
HKLM\SYSTEM\CurrentControlSet\Services\VirtualBox NDIS6 Bridged Networking Driver
HKLM\SYSTEM\CurrentControlSet\Services\VirtualBox System Service
```

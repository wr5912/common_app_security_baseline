...
本页记录 [[MongoDB]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。

## 2. 常见字段

```text
HKCU\SOFTWARE\MongoDB Inc.\*
HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*
HKLM\SOFTWARE\MongoDB Inc.\*
HKLM\SYSTEM\CurrentControlSet\Services\MongoDB
HKLM\SYSTEM\CurrentControlSet\Services\MongoDB Server
```

...
os: windows
hive: HKLM/HKCU
key_pattern: "Adobe Acrobat 注册表画像"
purpose:
  - service_config
  - app_config
  - uninstall_inventory
...
本页记录 [[Adobe Acrobat]] 相关服务、应用配置和卸载信息的注册表位置模式。服务类应用优先核验 `ImagePath`、`ObjectName`、`Start`、`Type`、`FailureActions` 和 `DelayedAutoStart`。
...
HKLM\SYSTEM\CurrentControlSet\Services\Adobe Acrobat Update Service
HKLM\SYSTEM\CurrentControlSet\Services\Adobe Genuine Monitor Service
HKLM\SYSTEM\CurrentControlSet\Services\Adobe Genuine Software Integrity Service
HKLM\SYSTEM\CurrentControlSet\Services\Adobe Update Service
HKLM\SYSTEM\CurrentControlSet\Services\AdobeARMservice
HKLM\SYSTEM\CurrentControlSet\Services\AdobeUpdateService
```

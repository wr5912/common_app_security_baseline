...
type: app
os: windows
app_id: app_wireguard
app_name_cn: "WireGuard"
...
WireGuard 属于 `VPN / 组网 / 代理` 场景应用。当前画像依据 `/tmp/windows系统上常见应用.md` 中的常见服务基线清单建立，用于终端服务、进程和资产角色识别；具体路径、签名和版本差异需要结合企业终端观测确认。

## 2. 常见安装路径

```text
C:\Program Files\WireGuard\wireguard.exe
C:\Program Files\WireGuard\wireguard-tunnel.exe
```
...
每个隧道可对应一个 WireGuardTunnel 服务。

## 11. 异常关注点

```text
隧道名、服务创建时间或对端地址不符合授权配置
服务由未知目录启动
```

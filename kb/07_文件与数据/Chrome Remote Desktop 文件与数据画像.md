...
type: file_artifact
os: windows
artifact_name: "Chrome Remote Desktop 文件与数据画像"
...
本页记录 [[Chrome Remote Desktop]] 的安装目录、配置、日志、缓存或用户态数据位置模式，用于判断路径、签名、落地时间线和数据访问是否符合应用画像。

## 2. 常见路径

```text
%APPDATA%\ChromeRemoteDesktop\\
%LOCALAPPDATA%\ChromeRemoteDesktop\\
%ProgramData%\ChromeRemoteDesktop\\

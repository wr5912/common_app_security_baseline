...
child_process: "[[vmware-usbarbitrator64.exe]]"
...
`services.exe` 启动 `vmware-usbarbitrator64.exe` 是 [[VMware Workstation]] 画像中的常见进程链路之一。判断时必须结合路径、签名、命令行、启动账户、用户交互、服务注册表和网络行为。

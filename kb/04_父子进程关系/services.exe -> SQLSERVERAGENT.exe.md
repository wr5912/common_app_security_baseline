...
child_process: "[[SQLSERVERAGENT.exe]]"
...
## 1. 关系说明
`services.exe` 启动 [[SQLSERVERAGENT.exe]] 是 [[Microsoft SQL Server]] 或其服务组件的常见服务启动链路。具体是否正常必须结合服务注册表、ImagePath、签名、启动账户、命令行和资产授权判断。

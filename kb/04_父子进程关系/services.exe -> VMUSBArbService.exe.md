...
child_process: "[[VMUSBArbService.exe]]"
...
## 1. 关系说明
`services.exe` 启动 [[VMUSBArbService.exe]] 是 [[VMware Workstation]] 或其服务组件的常见服务启动链路。具体是否正常必须结合服务注册表、ImagePath、签名、启动账户、命令行和资产授权判断。

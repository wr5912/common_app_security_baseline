...
type: service
os: windows
service_name: "VMware DHCP Service"
display_name: "VMnetDHCP"
app: "[[VMware Workstation]]"
vendor: "VMware"
service_type: Win32OwnProcess
start_type: unknown
start_account: LocalSystem
image_name: "vmnetdhcp.exe"
risk_level: medium
confidence: medium
status: active
tags:
...
`VMware DHCP Service` 是 `VMware Workstation` 相关的常见 Windows 服务名或服务名模式。第三方服务名可能随版本、安装方式和企业定制包变化，应结合路径、签名、启动账户和资产授权共同识别。
...
```text
HKLM\SYSTEM\CurrentControlSet\Services\VMware DHCP Service
```
...
- 来源行：`third-party-service-032`，第 385 行
- 所属应用：[[VMware Workstation]]
- 进程：[[VMnetDHCP.exe]]
- 父子关系：[[services.exe -> VMnetDHCP.exe]]
- 注册表：[[Windows 服务注册表画像]]

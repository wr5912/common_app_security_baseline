...
display_name: "SentinelAgent"
app: "[[SentinelOne]]"
vendor: "SentinelOne"
service_type: Win32OwnProcess
start_type: unknown
start_account: LocalSystem
image_name: "SentinelAgent.exe"
risk_level: medium
confidence: medium
status: active
tags:
...
`Sentinel Agent` 是 `SentinelOne` 相关的常见 Windows 服务名或服务名模式。第三方服务名可能随版本、安装方式和企业定制包变化，应结合路径、签名、启动账户和资产授权共同识别。
...
```text
HKLM\SYSTEM\CurrentControlSet\Services\Sentinel Agent
```
...
- 与 [[SentinelOne]] 的授权安装、更新、管理或业务功能一致。
...
- 来源行：`third-party-edr-007`，第 442 行
- 所属应用：[[SentinelOne]]
- 进程：[[SentinelAgent.exe]]
- 父子关系：[[services.exe -> SentinelAgent.exe]]
- 注册表：[[Windows 服务注册表画像]]

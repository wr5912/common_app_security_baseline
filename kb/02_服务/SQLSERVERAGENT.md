...
`SQL Server Agent` 是 `Microsoft SQL Server` 相关的常见 Windows 服务名或服务名模式。第三方服务名可能随版本、安装方式和企业定制包变化，应结合路径、签名、启动账户和资产授权共同识别。
...
- 来源行：`third-party-service-045`，第 403 行
- 所属应用：[[Microsoft SQL Server]]
- 进程：[[SQLSERVERAGENT.exe]]
- 父子关系：[[services.exe -> SQLSERVERAGENT.exe]]
- 注册表：[[Windows 服务注册表画像]]

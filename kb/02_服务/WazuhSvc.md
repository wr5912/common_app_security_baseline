...
`Wazuh Agent` 是 `Wazuh Agent` 相关的常见 Windows 服务名或服务名模式。第三方服务名可能随版本、安装方式和企业定制包变化，应结合路径、签名、启动账户和资产授权共同识别。
...
```text
HKLM\SYSTEM\CurrentControlSet\Services\Wazuh Agent
```
...
- 与 [[Wazuh Agent]] 的授权安装、更新、管理或业务功能一致。
...
- 来源行：`third-party-edr-022`，第 457 行
- 所属应用：[[Wazuh Agent]]
- 进程：[[WazuhSvc.exe]]
- 父子关系：[[services.exe -> WazuhSvc.exe]]
- 注册表：[[Windows 服务注册表画像]]
